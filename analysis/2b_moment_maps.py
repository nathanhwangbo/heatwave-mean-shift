...
depends on:
- src/changing_heat_extremes/flags.py for the config of the analysis (e.g. how "extreme heat" is defined, and what time periods to compare)
- processed data in processed_data/moments_ds_{flags.label}.nc, which is generated by analysis/2a_get_moments.py
"""
...
if flags.use_mean_shift:
    mean_label = "Mean Shift (°C)"
else:
    mean_label = "Median Shift (°C)"

horizontal_cbar_meanshift = phelpers.horizontal_cbar_hv(clabel=mean_label, shrink=0.7)
...
# hvplot.save(fig_meanshift, fig_dir / "supplemental" /  f"fig_shift_{flags.label}.png", dpi = 200)
