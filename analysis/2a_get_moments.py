...
#  (the processing is done in 0_era_meanshift.py)
#######################################################################

# anomalies calculated in 0_era_meanshift.py
era_land_anom = (
    xr.open_dataset(data_dir / f"land_anom_{flags.ref_years[0]}_{flags.ref_years[1]}.nc")
...
if flags.use_mean_shift:
    ...
    tmax_mean_diff = (era_land_new.median(dim="time") - era_land_old.median(dim="time")).rename(
        {"t2m_x": "t2m_x_mean_diff"}
    )
...
# NOTE! This comes from ERA daily TMAX which has been processed by:
#   - centered to have mean zero over the entire 1960-2025 period (or ref period, if flags.use_ref_years_for_climatology is True)
#   - removing individual yearly means, so that each year is mean 0 (removing warming signal)
#   - removing day-of-year means, so that the Jan 12 time series is mean 0.
# (the processing is done in 02_era_climatology.py)
##############################################
...
if flags.use_ref_years_for_climatology:
    clim_year0 = flags.ref_years[0]
    clim_year1 = flags.ref_years[1]
else:
    clim_year0 = flags.ref_years[0]
    clim_year1 = flags.new_years[1]


era_land_anom_for_climatology = (
    xr.open_dataset(data_dir / f"land_anom_for_climatology_{clim_year0}_{clim_year1}.nc")
    .chunk({"time": -1, "lat": 100, "lon": 100})
    .sortby(["lon", "lat"])
)

clim_skew = stats.skew(era_land_anom_for_climatology["t2m_x"], dims=["time"], dask="parallelized").rename("t2m_x_skew")
clim_kurt = stats.kurtosis(era_land_anom_for_climatology["t2m_x"], dims=["time"], dask="parallelized").rename(
    "t2m_x_kurt"
)
...
moments_path = data_dir / f"moments_ds_{flags.label}.nc"
# ! uncomment to write to file
if not moments_path.exists():
    print("writing moments dataset to file...")
    ahelpers.write_nc(combined_ds, moments_path)
